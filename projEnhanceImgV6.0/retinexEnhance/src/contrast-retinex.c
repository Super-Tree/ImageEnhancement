...
static void     retinex_scales_distribution (float       *scales,
                                             int          nscales,
                                             int          mode,
                                             int          s);

static void     compute_mean_var            (float       *src,
                                             float       *mean,
                                             float       *var,
                                             int          size,
                                             int          bytes);
...
static void     gausssmooth                 (float       *in,
                                             float       *out,
                                             int          size,
                                             int          rowtride,
                                             gauss3_coefs *c);
         ...
         GimpPreview  *preview)
{
  ...
  if (preview)
  ...
  else
    {
      ...
      src = g_try_malloc (sizeof (uchar) * size);
      ...
      memset (src, 0, sizeof (uchar) * size);
 ...
 */
static void
compute_coefs3 (gauss3_coefs *c, float sigma)
{
  /*
...
static void
gausssmooth (float *in, float *out, int size, int rowstride, gauss3_coefs *c)
{
  /*
   * Papers:  "Recursive Implementation of the gaussian filter.",
   *          Ian T. Young , Lucas J. Van Vliet, Signal Processing 44, Elsevier 1995.
  ...
  bufsize = size+3;
  size -= 1;
  w1 = (float *) g_try_malloc (bufsize * sizeof (float));
  w2 = (float *) g_try_malloc (bufsize * sizeof (float));
  w1[0] = in[0];
  w1[1] = in[0];
  w1[2] = in[0];
 ...
 */
static void
MSRCR (uchar *src, int width, int height, int bytes, gboolean preview_mode)
{
  ...
  int          pos;
  int          channel;
  uchar       *psrc = NULL;            /* backup pointer for src buffer */
  ...
  size = width * height * bytes;
  dst = g_try_malloc (size * sizeof (float));
  if (dst == NULL)
    {
      g_warning ("Failed to allocate memory");
      return;
  ...
  in  = (float *) g_try_malloc (channelsize * sizeof (float));
  if (in == NULL)
    {
      g_free (dst);
      g_warning ("Failed to allocate memory");
  ...
  out  = (float *) g_try_malloc (channelsize * sizeof (float));
  if (out == NULL)
    {
      g_free (in);
      g_free (dst);
 ...
 */
static void
compute_mean_var (float *src, float *mean, float *var, int size, int bytes)
{
  float vsquared;
  int i,j;
  float *psrc;
